 ...
 * 5. в методе render передаём в клавиатуру координаты касания и если
 *    touch вернул true, то завершаем ввод и передаём введённый текст
 *    в переменную name, а клавиатура исчезает
 *    if (keyboard.touch(touch.x, touch.y)) name = keyboard.getText();
 * 6. в batch рисуем клавиатуру, она будет рисоваться только после вызова
 *    метода keyboard.start()
 *    keyboard.draw(batch);
